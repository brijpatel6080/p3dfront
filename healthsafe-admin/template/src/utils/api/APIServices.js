...
//
export function getSpecialities(token, userType) {
  return new Promise(async (resovle, reject) => {
    const result = {
      response: {},
      error: "",
    ...
    try {
      const options = createOptions(token);
      const res = await axios.get("/specialties/role/" + userType, options);
      result.response = res.data;
      resovle(result);
    } catch (error) {
      result.error = error;
...
export function addhsp(payload, token) {
  return new Promise(async (resovle, reject) => {
    const result = {
      response: {},
      error: "",
    ...
    try {
      const options = createOptions(token);
      const res = await axios.post("/admin/add-hsp", payload, options);
      result.response = res.data;
      resovle(result);
    } catch (error) {
      result.error = error;
